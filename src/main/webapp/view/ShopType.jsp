...
<body>
 ...
 <li>
				...
				<li><a href="Cart?operation=6"><i class="fa fa-fw fa-file"></i>
						购物车</a></li>
					 <li>
...
<th>创建时间</th>
<th>更新时间</th>
<th colspan="3">类型操作</th>
</tr>
<c:forEach items="${pageModel.data}" var="Categorys">
<tr>
<td>${Categorys.id}</td>
<td>${Categorys.cname}</td>
<td>${Categorys.status}</td>
<td>${Categorys.sort_order}</td>
<td>${Categorys.create_time}</td>
<td>${Categorys.update_time}</td>
<td><a href="Category?operation=3&id=${Categorys.id}"  style="color: black" >删除</a></td>
<td><a href="Category?operation=1&id=${Categorys.id}" style="color: red">修改</a></td>
<td><a href="Category?operation=6&id=${Categorys.id}" >查看</a></td>  
</tr>
</c:forEach>
</table>
...
<a style="color: red" href="Category?pageNo=${pageNo}&operation=7">${pageNo}</a>
</c:when>
<c:when test="${pageModel.currentPage!=pageNo}">
<a href="Category?pageNo=${pageNo}&operation=7">${pageNo}</a>
</c:when>
</c:choose>
</c:forEach>
</div>
